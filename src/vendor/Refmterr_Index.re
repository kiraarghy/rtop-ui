...
let normalizeCompilerLineColsToRange = (~fileLines, ~lineRaw, ~col1Raw, ~col2Raw) => {
  ...
  if (numberOfCharsBetweenStartAndEndColumn <= numberOfCharsLeftToCoverOnStartingRow) {
    ...
    let suddenlyFunctionalProgrammingOutOfNowhere =
      ...
      |> listTakeWhile(numberOfCharsOnThisLine =>
...
let extractFromFileMatch = fileMatch =>
  Re.Pcre.(
    ...
    | [Delim(_), Group(_, filePath), Group(_, lineNum), col1, col2, Text(body)] =>
      let cachedContent = fileLinesOfExn(filePath);
...
let parse = (~customLogOutputProcessors, ~customErrorParsers, err) => {
  ...
  try (
    ...
    | [Re.Pcre.Delim(_), Group(_, filePath), Group(_, lineNum), col1, col2, Text(body)] =>
      ...
      switch (
        Refmterr_ParseError.specialParserThatChecksWhetherFileEvenExists(filePath, errorCapture)
      ) {
      | Some(err) => err
      | None =>
        let cachedContent = fileLinesOfExn(filePath);
        ...
        | (Some(errorBody), (None, None)) =>
          ErrorContent({
            ...
            range,
            parsedContent:
              Refmterr_ParseError.parse(~customErrorParsers, ~errorBody, ~cachedContent, ~range),
          })
        | (None, (Some(code), Some(warningBody))) =>
          ...
          Warning({
            ...
            parsedContent: {
              code,
              warningType:
                Refmterr_ParseWarning.parse(code, warningBody, filePath, cachedContent, range),
            },
          });
...
let parseFromRtop = (~customErrorParsers, ~content: string, ~error as err) => {
  ...
  | [Re.Pcre.Delim(_), Group(_, filePath), Group(_, lineNum), col1, col2, Text(body)] =>
    ...
    let cachedContent = split_on_char('\n', content);
    /* sometimes there's only line, but no characters */
    let (col1Raw, col2Raw) =
      switch (col1, col2) {
      | (Group(_, c1), Group(_, c2)) =>
