...
module ReportError = Refmterr_ReportError;
module ReportWarning = Refmterr_ReportWarning;
    ...
    if (i >= startRow && i <= endRow) {
      if (startRow == endRow) {
        ...
        let highlightedBeginning =
          highlightSource(~dim=true, stringSlice(~last=startColumn, currLine));
        ...
        let highlightedEnd =
          red(~bold=true, ~underline=true, stringSlice(~first=startColumn, currLine));
        let highlightedBeginning =
          highlightSource(~dim=true, stringSlice(~last=startColumn, currLine));
    ...
    if (startRow === endRow) {
      ...
      sp(
        ...
        highlight(
          ~dim=true,
          ~underline=true,
          sp(":%d:%d-%d:%d", startRow + 1, startColumn, endRow + 1, endColumn),
        ),
        labelColor(~bold=true, warningCodeStr),
      );
    };
