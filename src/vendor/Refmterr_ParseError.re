...
let functionArgsCount = str => {
  ...
  let cleaned = Re.Pcre.substitute(~rex=nestedFunctionTypeR, ~subst=_ => "|||||", str);
...
/* TODO: when it's a -> b vs b, ask if whether user forgot an argument to the
   func */
let incompatR = Re.Pcre.regexp({|([\s\S]*?)is not compatible with type([\s\S]*)|});
...
let extractTypeIncompatsFromExtra = extra => {
  ...
  let (remainingExtra, incompats) = List.fold_left(folder, ("", []), splitted);
...
let type_IncompatibleType = (err, _, range) => {
  ...
  let term = get_match_n(1, allR, err) == "expression has type" ? Expression : Pattern;
...
let type_UnboundValue = (err, _, _) => {
  ...
  let suggestions =
    ...
    |> optionMap(Re.Pcre.split(~rex=Re.Pcre.regexp({|, | or |})));
...
let type_SignatureItemMismatch = (err, cachedContent, _) => {
  ...
  | (Some(missingItemsText), _, _) =>
    let missingItems = stringNsplit(missingItemsText, ~by="Expected declaration");
    ...
    for (i in 0 to List.length(missingItems) - 1) {
      ...
      if (String.length(itm) > 0) {
        ...
        | (Some(fileOrType), Some(name), Some(fileName), Some(line)) =>
          ...
          missing.contents = [(what, name, fileName, line), ...missing.contents];
        | _ => ()
        };
      };
    };
  ...
  | (None, None, Some(_)) =>
    ...
    | (Some(good), Some(bad), Some(goodFile), Some(goodLn), Some(badFile), Some(badLn)) =>
      Type_SignatureItemMismatch({
...
let file_SyntaxError = (err, cachedContent, range) => {
  ...
  if (! Re.Pcre.pmatch(~rex=allR, err) && ! Re.Pcre.pmatch(~rex=allUnknownR, err)) {
    raise(Not_found);
  } else {
    ...
    File_SyntaxError({
      hint: optionMap(String.trim, hint),
      offendingString:
        stringSlice(~first=startColumn, ~last=endColumn, List.nth(cachedContent, startRow)),
    });
  };
};
